...
def run(mode="local", texts=None):
    ...
    serial = type(
        "S", (), {"frames": [], "write": lambda self, b: self.frames.append(b)}
    )()
    if mode == "g2":
        sink = G2GlassesSink(serial)
    elif mode == "pebble":
        sink = ConsoleSink()
    else:
        sink = LocalScreenSink(serial)
    dev = DeviceSim()
    batt = BatteryMonitor()
    ...
    g = gest.push(0.1, 0.9, 0.0)
    dev.gesture(g) if g else None
    mv = batt.read_mv()

...
def _make(action: str, name: str, desc: str, extra_props: dict, config: AgentConfig, session) -> Tool:
    ...
    def run(args: dict) -> str:
        ...
        try:
            ...
            resp = sess.post(url, data=json.dumps(payload).encode(),
                             headers={"Content-Type": "application/json"}, timeout=15)
            return json.loads(resp.text if hasattr(resp, "text") else "{}").get("result", "ok")

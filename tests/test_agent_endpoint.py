...
class FakeAgent:
    ...
    def run(self, text, images=None, audio_transcript=None):
        return type("R", (), {"text": f"agent reply to: {text[:20]}",
                              "tool_calls": 1,
                              "steps": [{"tool": "device", "args": {"action": "notes"},
...
def test_api_agent_returns_steps():
    with tempfile.TemporaryDirectory() as d:
        ...
        try:
            url = f"http://127.0.0.1:{port}/api/agent?text=" + urllib.parse.quote("summarize my day")

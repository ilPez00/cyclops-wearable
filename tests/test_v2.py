...
def test_hud_roundtrip():
    ...
    d = parse_hud(f)
    assert (
        d["kind"] == 0 and d["lines"] == ["Buy milk", "Call mom"] and d["more"] is True
    )
...
def test_health_join():
    ...
    if os.path.exists("/tmp/cyclops_health_test.jsonl"):
        os.remove("/tmp/cyclops_health_test.jsonl")
    hs = HealthStore("/tmp/cyclops_health_test.jsonl")
...
def test_ring_client():
    ...
    if os.path.exists("/tmp/cyclops_ring_test.jsonl"):
        os.remove("/tmp/cyclops_ring_test.jsonl")
    rc = RingClient(HealthStore("/tmp/cyclops_ring_test.jsonl"))
...
def test_pipeline_health_enrich():
    if os.path.exists("/tmp/cyclops_ph.jsonl"):
        os.remove("/tmp/cyclops_ph.jsonl")
    if os.path.exists("/tmp/cyclops_pn.jsonl"):
        os.remove("/tmp/cyclops_pn.jsonl")
    hs = HealthStore("/tmp/cyclops_ph.jsonl")
    now = int(time.time() * 1000)
    hs.add(HealthSample(t=now, hr=90))
...
def test_peer_hello():
    ...
    assert (
        b'"peer": "bead"'.split(b" ")[1] in b
        or b'"peer": "bead"'.replace(b" ", b"") in b
    )

...
PAT_REMINDER = re.compile(r"\b(remind me to|reminder to|don't forget to|remember to)\b", re.I)
PAT_TASK = re.compile(r"\b(todo|to-do|i need to|i have to|i must|let's|lets|we should|i will|need to)\b", re.I)
PAT_IDEA = re.compile(r"\b(idea|what if|maybe we|suggest|could we)\b", re.I)
PAT_IDEA_LEAD = re.compile(r"^\s*(idea)\s*:\s*", re.I)
PAT_DECISION = re.compile(r"\b(we decided|decided to|we will|agreed to|the plan is|conclusion:)\b", re.I)
...
def _resolve_due(text: str) -> str | None:
    t = text.lower()
    today = datetime.now()
    if "tomorrow" in t: return (today + timedelta(days=1)).date().isoformat()
    if "today" in t: return today.date().isoformat()
    if "next week" in t: return (today + timedelta(weeks=1)).date().isoformat()
    ...
    if m: return f"{today.year}-{int(m.group(2)):02d}-{int(m.group(1)):02d}"
    return None
